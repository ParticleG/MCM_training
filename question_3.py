...
def check_disease(_total_population, _infection_rate, _previous_group_size, _positive_group_cumulated,
    ...
    _positive_group_count = (
            Dec(_total_population) / Dec(_next_population) * ((Dec(1) - _negative_expect / _next_population) +  # Original positive groups
                                                              ...
                                                              _false_positive_rate * (Dec(1) - _false_negative_rate) * (Dec(1) - _recessive_rate) +
                                                              # Positive groups in second check
                                                              (Dec(1) - _false_negative_rate) * (
                                                                      (Dec(1) - _false_negative_rate) * _recessive_rate + _false_negative_rate))
    )
    _next_infection_rate = Dec(_infection_rate) / (Dec(1) - (Dec(1) - _false_positive_rate - _false_negative_rate - _recessive_rate) * (
